    ...
    */
    _self.Bind = function() {
        document.getElementById(_self.Setting.UpBtn).onchange = function() {
        	var ai = this.value.lastIndexOf("\\");
        	if(ai < 0){
        		ai = this.value.lastIndexOf("/");
        	}
        	var imgName = this.value.substring(ai + 1);
            if (this.value) {
                if (!RegExp("\.(" + _self.Setting.ImgType.join("|") + ")$", "i").test(this.value.toLowerCase())) {
                	alert(_self.Setting.FileType+"格式不正确！");
                    this.value = "";
                    return false;
                }
                ...
                /*if(this.files && this.files[0]){
	                var size =  this.files[0].fileSize || this.files[0].size; 
	                
	    			if(size > 1 * 1024 * 1024){
	    				$.messager.alert("来自网页的消息","保存失败，"+_self.Setting.FileType+"大小不符合要求！","info");
	    				this.value = "";
	    				return false;
	    			}
                }*/
